# THYAO.IS Stock Risk Report

This report includes the risk metrics calculated using purely deterministic Python code, based on the last 2 years of daily closing prices for **THYAO.IS** fetched via yfinance.

## Calculated Risk Metrics

| Risk Metric | Calculated Value | Description |
| :--- | :--- | :--- |
| **Annualized Volatility** | 31.43% | Annualized daily standard deviation of log returns (based on 252 trading days). Represents price variance. |
| **Historical VaR (95% Confidence)** | 2.51% | Indicates that daily loss will not exceed this rate with 95% probability based on historical data. |
| **Historical VaR (99% Confidence)** | 5.10% | Indicates that daily loss will not exceed this rate with 99% probability based on historical data (extreme risk). |
| **Sharpe Ratio (Rf = 0%)** | 0.0993 | Annualized log returns divided by annualized volatility. Performance per unit of risk. |
| **Maximum Drawdown (Max DD)** | -24.40% | The largest percentage loss recorded from peak to trough over the last 2 years. |
...
![Monte Carlo 1-Day Distribution Chart](THYAO_IS_mc_returns_histogram.png)
