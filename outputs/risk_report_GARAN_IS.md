# GARAN.IS Stock Risk Report

This report includes the risk metrics calculated using purely deterministic Python code, based on the last 2 years of daily closing prices for **GARAN.IS** fetched via yfinance.

## Calculated Risk Metrics

| Risk Metric | Calculated Value | Description |
| :--- | :--- | :--- |
| **Annualized Volatility** | 38.87% | Annualized daily standard deviation of log returns (based on 252 trading days). Represents price variance. |
| **Historical VaR (95% Confidence)** | 3.48% | Indicates that daily loss will not exceed this rate with 95% probability based on historical data. |
| **Historical VaR (99% Confidence)** | 6.21% | Indicates that daily loss will not exceed this rate with 99% probability based on historical data (extreme risk). |
| **Sharpe Ratio (Rf = 0%)** | 0.4365 | Annualized log returns divided by annualized volatility. Performance per unit of risk. |
| **Maximum Drawdown (Max DD)** | -27.81% | The largest percentage loss recorded from peak to trough over the last 2 years. |
...
### Monte Carlo 1-Day Simulated Return Distribution (GARAN.IS)
The distribution of returns obtained from the simulation and the Monte Carlo VaR values are shown in the chart below:

![Monte Carlo 1-Day Distribution Chart](GARAN_IS_mc_returns_histogram.png)
